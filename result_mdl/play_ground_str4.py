...
class Str4_1_1:
    ...
    def get_counts(self):
        if type(self.share_tick) == str:
            self.share_tick = Base_mdl(en_4_1(ts_4_1(self.market_tick, self.share_tick,
                                                     self.stime, self.ftime, self.interval))).deals_counts()
            rs = ''
        else:
            for i in range(len(self.share_tick)):
                try:
...
class Str4_2_2:
    ...
    def signals(self):
        ...
        """ !!! ADD LAST MONTH (NOW JUNE 6) !!! """
        if type(self.share_tick) == str or None:
            x = Base_mdl(en_4_2(ts_4_2(self.market_tick, self.share_tick,
                                                self.stime, self.ftime, self.interval))).signal2()
        else:
            copy_names = self.share_tick.copy()
            for i in range(len(self.share_tick)):
                try:
                    self.share_tick[i] = Base_mdl(en_4_2(ts_4_2(self.market_tick, self.share_tick[i],
                                                                  self.stime, self.ftime, self.interval))).signal2()
                    d = {"Companies": copy_names, 'Signals': self.share_tick}
                    print(d)
                except:
    ...
    def sum_results(self):
        if type(self.share_tick) == str:
            self.share_tick = Base_mdl(en_4_2(ts_4_2(self.market_tick, self.share_tick,
                                                    self.stime, self.ftime, self.interval))).rt2()
            rs = ''
        else:
            for i in range(len(self.share_tick)):
    ...
    def get_table(self):
        return en_4_2(ts_4_2(self.market_tick, self.share_tick, self.stime, self.ftime, self.interval))



    def month_results(self):
        return Base_mdl(en_4_2(ts_4_2(self.market_tick, self.share_tick,
                                        self.stime, self.ftime, self.interval))).date_sum2()
    ...
    def month_results_without_bad(self):
        ...
        if type(self.share_tick) == str:
            ...
            for i in range(len(self.share_tick)):
                try:
                    self.share_tick[i] = list(Base_mdl(en_4_2(ts_4_2(self.market_tick, self.share_tick[i],
                                                                  self.stime, self.ftime, self.interval))).del_any_months())
                except:
                    self.share_tick[i] = 0
            rs = sum(self.share_tick)
...
if __name__ == '__main__':
    ...
    print(Str4_2_2('^GSPC', 'T').get_table())