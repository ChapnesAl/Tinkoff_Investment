...
class Str4_0_0:
    ...
    def signals(self):
        ...
        if type(self.share_tick) == str or None:
            x = Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick))).signal2()
        else:
            copy_names = self.share_tick.copy()
            for i in range(len(self.share_tick)):
                try:
                    self.share_tick[i] = (Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick[i]))).signal2())
    ...
    def sum_results(self):
        if type(self.share_tick) == str:
            self.share_tick = Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick))).rt2()
            rs = ''
        else:
            for i in range(len(self.share_tick)):
                try:
                    self.share_tick[i] = Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick[i]))).rt2()
                except:
                    self.share_tick[i] = 0
    ...
    def month_results(self):
        return Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick))).date_sum2()

    def month_results_without_bad(self):
        """ WITH LAST CURRENT MONTH (6 now) """
        if type(self.share_tick) == str:
            self.share_tick = Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick))).del_any_months()
            rs = ''
        else:
            for i in range(len(self.share_tick)):
                try:
                    self.share_tick[i] = list(Base_mdl(en_4_0(ts_4_0(self.market_tick, self.share_tick[i]))).del_any_months())
                except:
                    self.share_tick[i] = 0
            rs = sum(self.share_tick)
        return self.share_tick, rs
